...
/*
	...
	For example, suppose we have a package called store that defines an interface called Store for storing and retrieving data. If we declare the Store interface in the store package, users of the store package can see exactly what methods they can call on types that implement the Store interface.

	On the other hand, if we declare the Store interface in a separate package where it is implemented, users of the store package would not be able to see the methods of the Store interface unless they imported the implementation package. This could make it more difficult for users of the store package to understand how to use types that implement the Store interface.
